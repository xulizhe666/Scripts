...
let disneyPolicyName = $.getval('Helge_0x001.Disney_Policy') || 'Disney+'
let debug = $.getval('Helge_0x001.Disney_Debug') === 'true'
let recheck = $.getval('Helge_0x001.Disney_Recheck') === 'true'
let t = parseInt($.getval('Helge_0x001.Disney_Timeout')) || 8000
let sortByTime = $.getval('Helge_0x001.Disney_Sort_By_Time') === 'true'
let concurrency = parseInt($.getval('Helge_0x001.Disney_Concurrency')) || 10
...
;(async () => {
  ...
  if (!isValidPolicy(policies[disneyPolicyName])) {
    ...
    $.setval(disneyPolicyName, 'Helge_0x001.Disney_Policy')
  }
  ...
  if (status === STATUS_AVAILABLE) {
    ...
    $.msge($.name, `${actualNode}`, `当前节点支持 Disney+ ➟ ${flag} ${regionName}`)
    return
  }

  let cachePolicies = []
  try {
    cachePolicies = JSON.parse($.getval('Helge_0x001.Disney_Available_Policies') ?? '[]')
  } catch (error) {
  ...
  $.setval(JSON.stringify(cachePolicies), 'Helge_0x001.Disney_Available_Policies')
